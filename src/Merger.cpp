...
constexpr double Merger::DEFAULT_MERGE_DST;
constexpr double Merger::DEFAULT_COLOR_DELTA;
            ...
            + pow(input_list.objects[i].cielab[2] - input_list.objects[j].cielab[2], 2) < color_delta)
        {
          ...
          if (min_sqr_dst < merge_dst)
          {
            ...
            *merged_cloud = *obj1_cloud + *obj2_cloud;
            ...
            // note: merging the data in image_indices will only make sense if the segmented objects were calculated
            // from the same rgbd data, although if this is being used at all then the developer should already be
            // aware of this
            merged_object.image_indices = input_list.objects[i].image_indices;
            merged_object.image_indices.insert(merged_object.image_indices.end(),
                input_list.objects[j].image_indices.begin(), input_list.objects[j].image_indices.end());
